<?php
	 ...
	 if(isset($_POST['submitlogin'])){
			 ...
			 if($result = mysqli_query($conn,$sql)){
						 ...
						 if(mysqli_num_rows($result) == 1){
	                         ...
	                         $_SESSION['givenName'] = $rows['user_f_name'];
							 			if( $rows['user_email'] == "sandesht801@gmail.com" ){
											 		if( isset($_POST['remember']) ){
					 		   	                       setcookie("email",$_POST['email'],time()+7*24*60*60 );
					 		   	                       setcookie("givenName",$rows['user_f_name'],time()+7*24*60*60);
					 		   	                       	setcookie("familyName",$rows['user_l_name'],time()+7*24*60*60);
					 		   	                       	setcookie("role",$rows['role'],time()+7*24*60*60);
					 		   	                       	setcookie("gender",$rows['user_gender'],time()+7*24*60*60);
												...
												header('Location: index.php?noset');
		 ...
		 echo header('Location: index.php?submitwrong');
	 }


	 if(isset($_POST['cart']) && isset($_SESSION["shopping_cart"]) ){
	 		if( isset($_POST['remember']) ){
					 	    setcookie("email",$_POST['email'],time()+7*24*60*60 );
					 		setcookie("givenName",$rows['user_f_name'],time()+7*24*60*60);
					 		setcookie("familyName",$rows['user_l_name'],time()+7*24*60*60);
				...
				echo header('Location: paypage.php');
	 }
?>